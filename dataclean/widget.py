...
def is_categorical(series, categorical_threshold=0.8):
    ...
    fraction_categorical = (
        ((type_counts[str] + type_counts[bool]) / sum(type_counts.values()))
...
class StepWidgetControllerBase(object):
    ...
    def create_widgets(self):
        ...
        self.submit_button = ipywidgets.Button(description="Add to Pipeline")
...
class NullReplaceWidgetController(StepWidgetControllerBase):
    ...
    def create_widgets(self):
        ...
        self.null_replace_selector.observe(self._update_step, names="value")
    ...
    def reset_controls(self, categorical_type):
        ...
        allowed_transforms = {
            x.value: x
            for x in ALLOWED_TRANSFORMATIONS[categorical_type]
...
class OutlierReplaceWidgetController(StepWidgetControllerBase):
    ...
    def create_widgets(self):
        ...
        self.outlier_range_slider.observe(self._update_step, names="value")
        self.outlier_replace_selector.observe(self._update_step, names="value")
    ...
    def update_step(self):
        ...
        self.cut_text.value = "{0} of {1} ({2:.0f}%) selected".format(
            num_values_cut, len(self.column), percent_values_cut
        )
...
class TypeConvertWidgetController(StepWidgetControllerBase):
    ...
    def load_data(self, column, numerical_data):
        ...
        for (data_type, count) in (
            self.column.dropna().apply(type).value_counts().iteritems()
        ):
    ...
    def create_widgets(self):
        ...
        self.type_selector.observe(self._update_step, names="value")
        self.replace_selector.observe(self._update_step, names="value")
        ...
        self.widget = ipywidgets.VBox(
            [
                ...
                ipywidgets.HBox(
                    [
                        ipywidgets.VBox(
                            [self.type_selector, self.replace_selector]
                        ),
                        self.submit_button,
                    ]
                ),
    ...
    def reset_controls(self, categorical_type):
        ...
        counts = reversed(
            sorted(self.type_count_dict, key=self.type_count_dict.get)
        )

        current_type = next(counts)

        while current_type not in self.type_selector.options.values() or (
            current_type is str
...
class RbmWidgetController(StepWidgetControllerBase):
    ...
    def create_widgets(self):
        ...
        title = ipywidgets.Label(
            value="Impute missing data with " "a Restricted Boltzmann Machine"
        )

        self.col_list = ipywidgets.SelectMultiple(
            options=[], description="On columns "
        )

        self.col_list.observe(
            lambda _: self._reload_categorical_list_options(
                self.categorical_list.options, index=self.col_list.index
            )
        )
        ...
        if isinstance(step, RbmStep):
            ...
            for col in self.col_list.options:
                if col in step.numerical_columns:
                    categorical_list.append(CategoricalTypes.CONTINUOUS.value)
                elif col in step.categorical_columns:
                    categorical_list.append(CategoricalTypes.CATEGORICAL.value)
                else:
            ...
            self._reload_categorical_list_options(
                categorical_list, index=self.col_list.index
            )
            self.step = step
        elif step:
...
class PlotWidgetController(object):
    ...
    def __init__(self):
        self.output_widget = ipywidgets.Output(
            layout=ipywidgets.Layout(min_width="300px", height="160px")
        )
        self.create_figure()
    ...
    def create_figure(self):
        ...
        self.ax_mod = self.fig.add_subplot(self.gs_two_plots[1])
    ...
    def draw_main_plot(self):
        ...
        if self.categorical_type is CategoricalTypes.CATEGORICAL:
            ...
            if len(col) > 0:
                col.sort_index().plot(kind="bar", ax=self.ax_main, alpha=0.4)
        else:
            hist_orig, self.bins = np.histogram(self.numerical_data)
            self.bin_width = self.bins[1] - self.bins[0]

            margin = (self.bins[-1] - self.bins[0]) * self.ax_main.margins()[0]
        ...
        self.low_cut_line, = self.ax_main.plot(
            [None, None], [self.ymax, 0], color=self.CUT_LINE_COLOUR
        )
    ...
    def draw_modified_plot(self, col_mod):
        ...
        data_mod = col_mod.loc[
            col_mod.apply(lambda x: isinstance(x, (int, float)))
        ]
        data_mod = data_mod.dropna()
        col_mod = col_mod.dropna().value_counts()

        if self.categorical_type is CategoricalTypes.CATEGORICAL and not self.column.dropna().value_counts().equals(
            col_mod
        ):
    ...
    def draw_cut_plot(self, low_cut, high_cut):
        ...
        cut_data = self.numerical_data.loc[
            self.numerical_data.apply(lambda x: x < low_cut or x > high_cut)
        ]
...
class ColumnWidgetController(object):
    ...
    def create_widgets(self):
        ...
        self.preview_widget_container = ipywidgets.VBox(
            [ipywidgets.Label(value="Current Step"), self.preview_widget],
    ...
    def categorical_selector_onchange(self, _):
        ...
        self.active_step = NullRemovalStep(
            replacement_method=NullRemovalMethod.NONE, colname=self.colname
        )
    ...
    def load_data(self, series, dataframe, step=None):
        ...
        self.numerical_data = series.loc[
        ...
        for controller in self.step_creation_controls:
            controller.load_data(
                column=self.column, numerical_data=self.numerical_data
            )
    ...
    def redraw_preview(self, col_modified=None):
        ...
        if col_modified is not None:
            col_mod = col_modified.reindex(
                index=self.column.index, fill_value="<br>"
            )
        else:
            col_mod = self.column

        self.preview_widget.value = (
            "<center>This Step</center>"
            + pd.concat(
                [self.column.rename("before"), col_mod.rename("after")], axis=1
            )
            .style.set_table_attributes('class="table"')
            .render()
        )
    ...
    def reset_controls(self):
        ...
        for transform_type in sorted(allowed_transforms, key=str):
            ...
            tab_titles.append(self.controls_dict[transform_type].tab_title)
        ...
        for controller in self.step_creation_controls:
            controller.reset_controls(categorical_type=self.categorical_type)

        self.categorical_selector.disabled = False
        self.categorical_selector.value = self.categorical_type

        self.plot_widget_controller.reset_plots(self.categorical_type)
...
class DataFrameWidgetController(object):
    ...
    def __init__(self, pipeline_widget, sampled_rows):
        ...
        if sampled_rows:
            ...
            child_widgets = [
                ipywidgets.HBox([sample_label, sample_btn])
            ] + child_widgets
...
class PipelineWidgetController(object):
    ...
    def render_widget(self, active_step=None):
        ...
        if children:
            children.append(
                ipywidgets.VBox(
                    [self.add_button, self.execute_button, self.export_button],
    ...
    def _enter_edit_mode(self, step):
        if step:
            self.add_button.layout.visibility = (
                None
    ...
    def _execute_pipeline(self):
        self.display_message("Executing pipeline... ")
